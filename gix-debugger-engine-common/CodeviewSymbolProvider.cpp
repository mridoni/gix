...
std::string CodeviewSymbolProvider::dumpStackFrame(GixDebugger *gd, void *hproc, void *hthread)
{
#if 1
...
#error There's something wrong with the build environment
	...
	do {
		...
		if (bSuccess) {
			res += std::string("%1 - %2:%3\n").arg(std::string::fromLocal8Bit(pSymbol->Name)).arg(line.FileName).arg(line.LineNumber);
		}
		else {
			res += std::string("%1\n").arg(std::string::fromLocal8Bit(pSymbol->Name));
		}
...
bool CodeviewSymbolProvider::initialize(GixDebugger *gd, void *hproc, void *userdata)
{
	...
	std::filesystem::path p(gd->getWorkingDirectory());
	std::string wd = p.make_preferred().string();
#if _DEBUG	
	//SymSetOptions(SYMOPT_DEBUG);
#endif
	return SymInitialize(hproc, wd.c_str(), false);
}
...
void *CodeviewSymbolProvider::loadSymbols(GixDebugger *gd, void *hproc, void *hmod, const std::string &mod_path, void *userdata, int *err)
{
	HANDLE hProcess = (HANDLE)hproc;
	DWORD64 dll_base = (DWORD64)hmod;

	DWORD64 hSymbols = SymLoadModule64(hProcess, NULL, mod_path.c_str(), 0, dll_base, 0);
	if (!hSymbols) {
		gd->printLastError();
		*err = GetLastError();
		return nullptr;
...
bool CodeviewSymbolProvider::unloadSymbols(GixDebugger *gd, void *hproc, void *hmod, const std::string &mod_path, void *userdata, int *err)
{
	return SymUnloadModule64((HANDLE)hproc, (DWORD64)hmod);
}

SharedModuleInfo *CodeviewSymbolProvider::extractModuleDebugInfo(GixDebugger *gd, void *hproc, void *hmod, void *hsym, const std::string &mod_path, void *userdata, int *err)
{
	HANDLE hProcess = (HANDLE)hproc;
	DWORD64 dll_base = (DWORD64)hmod;
	...
	for (auto src_mask : gd->src_file_types) {
		if (!SymEnumSourceFiles(hProcess, (ULONG64)hsym, src_mask.c_str(), DLL_EnumSourceFilesProc, mi)) {
			gd->printLastError();
			*err = -1;
			return nullptr;
		}
	...
	for (auto sf : mi->source_files) {
		SymEnumLines(hProcess, (ULONG64)hsym, NULL, sf.c_str(), DLL_EnumLinesProc, mi);
	}
	...
	*/
	...
	if (!__GIX_SYM_MODULES || __GIX_SYM_MODULES_C < 0 || __GIX_SYM_MODULES_S < 0) {
		spdlog::error("ERROR: cannot parse module debug info (module info)");
		delete mi;
		*err = -1;
		return nullptr;
	}
	...
	for (auto m : cml) {
		...
		cmi->name = m;
		cmi->owner = mi;
		cmi->entry_point = (void *)getSymbolAddress(gd, hproc, hmod, m, NULL, err);
...
/*
		...
		if (SymGetLineFromAddr64(hproc, (DWORD64)cmi->entry_point, &dwDisplacement, &ln)) {
			std::string c_src_file = ln.FileName;	// These are C source locations
			mi->owner->
...
*/
		...
		if (!initCobolModuleLocalInfo(gd, hproc, cmi)) {
			spdlog::error("ERROR: cannot parse module debug info (source lines)");
			delete cmi;
			delete mi;
			*err = -1;
			return nullptr;
	...
	if (!filterPreprocessedBlockBreakpoints(mi)) {
		spdlog::error("Cannot apply preprocessed block info");
		*err = -1;
		return nullptr;
	}
...
LibCobInfo *CodeviewSymbolProvider::extractLibCobInfo(GixDebugger *gd, void *hproc, void *hmod, void *hsym, const std::string &mod_path, void *userdata, int *err)
{
...
void * CodeviewSymbolProvider::getSymbolAddress(GixDebugger *gd, void *hproc, void *hmod, const std::string &sym_name, void *userdata, int *err)
{
	HANDLE hProcess = (HANDLE)hproc;
	SYMBOL_INFO *pSymbol;
	pSymbol = (SYMBOL_INFO *)new BYTE[sizeof(SYMBOL_INFO) + MAX_SYM_NAME];
	pSymbol->SizeOfStruct = sizeof(SYMBOL_INFO);
	pSymbol->MaxNameLen = MAX_SYM_NAME;
	BOOL rc = SymFromName(hProcess, sym_name.c_str(), pSymbol);
...
int CodeviewSymbolProvider::readSymbolValueAsInt(void *hproc, const std::string &s)
{
	void *addr = getSymbolAddress(NULL, hproc, NULL, s, NULL, 0);
	if (!addr) {
		spdlog::error("Cannot locate symbol [{}]", s);
		return -1;
	}
...
uint8_t *CodeviewSymbolProvider::readSymbolValueInBuffer(void *hproc, const std::string &s, int bfrlen)
{
	if (!bfrlen)
		return 0;
...
bool CodeviewSymbolProvider::initCobolModuleLocalInfo(GixDebugger *gd, void *hproc, CobolModuleInfo *cmi)
{
	...
	if (__GIX_SYM_MOD_EC < 0|| __GIX_SYM_MOD_ES < 0 || __GIX_SYM_MOD_MC < 0 || __GIX_SYM_MOD_MS < 0 || !__GIX_SYM_MOD_E || !__GIX_SYM_MOD_M) {
		spdlog::error("ERROR: cannot parse module debug info (COBOL variable data)");
		return false;
	}
...
BOOL __stdcall  DLL_EnumSourceFilesProc(PSOURCEFILE pSourceFile, PVOID UserContext)
{
	...
	if (mi)
		mi->source_files.push_back(std::string(pSourceFile->FileName));
